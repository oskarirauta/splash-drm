/*
 * socket.c - Abstract UNIX socket management with JSON replies
 */
        ...
        if (errno == EADDRINUSE) {
            ...
            if (test_fd >= 0) {
                if (connect(test_fd, (struct sockaddr *)&addr, sizeof(addr)) == 0) {
                    close(test_fd);
                    fprintf(stderr, "Another splash-drm instance is running\n");
                    close(st->server_fd);
                    st->server_fd = -1;
                    return -1;
                }
                close(test_fd);
                int opt = 1;
                setsockopt(st->server_fd, SOL_SOCKET, SO_REUSEADDR, &opt, sizeof(opt));
                if (bind(st->server_fd, (struct sockaddr *)&addr, sizeof(addr)) < 0) {
                    perror("bind retry");
                    close(st->server_fd);
                    st->server_fd = -1;
...
static int accept_client(splash_state_t *st) {
    ...
    for (int i = 0; i < MAX_SOCKET_CLIENTS; i++) {
        if (st->client_fds[i] < 0) {
            ...
            st->client_cmd_len[i] = 0;
            if (st->debug) {
                fprintf(stderr, "[debug] Client connected, slot %d (fd=%d)\n", i, fd);
            }
            return i;
        }
    }
...
static void close_client(splash_state_t *st, int idx) {
    if (idx < 0 || idx >= MAX_SOCKET_CLIENTS) return;
    if (st->client_fds[idx] >= 0) {
        if (st->debug) {
            fprintf(stderr, "[debug] Client disconnected, slot %d (fd=%d)\n", idx, st->client_fds[idx]);
        }
...
static int find_client_idx(splash_state_t *st, int fd) {
    for (int i = 0; i < MAX_SOCKET_CLIENTS; i++) {
        if (st->client_fds[i] == fd) return i;
    }
    return -1;
}
...
void socket_reply_json(splash_state_t *st, int client_idx, cJSON *response) {
    if (client_idx < 0 || client_idx >= MAX_SOCKET_CLIENTS) return;
    if (st->client_fds[client_idx] < 0) return;
        ...
        if (fds[i].revents & POLLIN) {
            ...
            for (ssize_t j = 0; j < n; j++) {
                ...
                if (buf[j] == '\n') {
                    ...
                    if (st->debug) {
                        fprintf(stderr, "[debug] JSON from client %d: %s", idx, st->client_cmd_buf[idx]);
                    }
