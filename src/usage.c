...
void print_cmd_help(const char *cmd) {
	if (strcmp(cmd, "text") == 0) {
		...
		"  remove_at_full  bool   delete the bar once it is full (default false =\n"
		"                         stay). Shows the full bar one frame then removes\n"
		"                         it; waits for smooth to finish first\n"
		...
		"  fade        bool/color  fade the screen out over the timeout window:\n"
		"                      true = black, or a colour like \"#101418\"\n\n"
		...
		"  fade        bool/col cross-fade into the cleared scene instead of\n"
		"                       cutting: true fades to the clear colour, a\n"
		"                       \"#rrggbb\" fades to that colour. The old frame\n"
		"                       fades out over the fresh one, so anything sent\n"
		"                       next is built underneath and revealed at the end\n"
		"  timeout     int      fade duration in seconds (default 1; with fade)\n\n"
